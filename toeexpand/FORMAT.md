...
## Corpus sweep 2026-06-01 — shipped OPSnippets (build 2025.32424)

Generated by `scripts/build-corpus.sh`, which expands **every `.tox` shipped in the TouchDesigner install** (`$TD_APP/Contents/Resources/tfs`, 788 files — the per-operator `OPSnippets/Snippets/` library across all 7 families plus system/dialog components) into `toeexpand/resources/shipped/` (gitignored, regenerable). Analysed with `python -m tocdir census`. All findings are build **2025.32424**, `osname Windows` (the snippets are Windows-authored — so the [Case-collision suffix mismatch](#case-collision-suffix-mismatch-toeexpand-bug) does occur here, on `n`/`parm` siblings).

**Round-trip validation — strongest to date:** `Project.from_dir(d).verify(d)` over **all 788 trees produced 0 mismatches** (210K `.n`, 197K `.parm`, plus 50K `.panel` / 36K `.text` / 21K `.cparm` / 14K `.table` and the long tail below). The parser is bit-exact across the entire shipped operator set, and **565 distinct `FAMILY:type` operator types** are now exercised (vs the prior corpus's narrower set), including the full `POP:` family.

`toeexpand` note: the CLI **exits 1 even on a fully successful expansion** (it prints the success message and writes valid output). Judge success by whether the `.dir/` was produced, not by exit code — `build-corpus.sh` does this.

### Newly observed file kinds (not previously in this doc)

All round-trip byte-exact as raw blobs; first-byte signatures recorded for follow-up decoding. Brace-block kinds share the `{ rate / start / tracks }` grammar already documented for `.script`/`.ts`/`.logic`.

| kind | count | signature (first bytes) | inferred meaning |
|---|---:|---|---|
| `.oscin` | 132 | `1\n{ rate=60 … }` | OSC In CHOP/DAT track state (brace-block family) |
| `.actorType` | 113 | `1\n2\n` | Bullet/physics Actor COMP type marker (version + int) |
| `.particle` | 31 | `1\n1\n0\n` | Particle SOP saved state (version + ints) |
| `.assetnames` | 13 | `1\n<count>\n0\n/Humanoid/Humanoid\n1\nrun\n1\npunch…` | animation/asset clip-name table (FBX/anim COMP) |
| `.cacheable` | 13 | `1\n` | cache-flag marker (version only) |
| `.subFiles` | 13 | `0\n` | sub-file manifest (minimal) |
| `.record` | 11 | `3\n{ rate=60 … }` | recorder state (brace-block family) |
| `.init` | 10 | `type = COMP:container\n` | component init/clone template (`key = value`) |
| `.def` | 10 | `tile … \nflags =  picked on …` | `.n`-style node-definition fragment (master/default def) |
| `.web` | 10 | `1\n*<u32×6><body>` | **Web DAT cached remote response** — standard DAT preamble; sample held a fetched crypto-market JSON. Security-relevant: marks a Web/HTTP-fetching DAT. |
| `.vfs` | 10 | `\0\0\0\x01\0\0\0\x01\0\0\0\x03 "<orig-path>" <vfs-name>\0<bytes>` | **Virtual File System** — embeds arbitrary files inside the `.toe`/`.tox` (sample embedded `C:/…/Banana.tif`). Security-relevant: a component can smuggle files of any type. |
| `.bind` | 6 | `1\n0\n` | parameter-bind table (minimal) |
| `.mergedpaths` | 6 | `1\n` | merged-op path manifest (minimal) |
| `.chn` | 5 | `{ channel <name> { lefttype = … } }` | Animation COMP channel/keyframe data (brace-block) |
| `.enginemap` | 5 | `1\n<count>\n<conn-name>\nop/<internal>\n…` | **Engine COMP** external-connector → internal-op map |
| `.enc` | 2 | `\0\0\0\x02 3 0 "" "" <ints>` | encrypted/encoded store (seen on `keymanager.tox`); opaque |

(Already documented as body kinds but still parser-`raw` — expected, not gaps: `.gnode`, `.replicator`, `.opfind`, `.oldacbo`, `.timeStateSetting`, `.geopaths`, `.textureimports`, `.learnedparms`, `.pluginstate`, `.pointfilein`, `.beat`, `.feedback`, `.timer`, `.grps`.)

### Security-relevant operator findings (for the Phase-2 exec-vector scanner)

- **Engine COMP is fully detectable statically.** `COMP:engine` node + `.parm` `file 0 <name>.tox` names the nested component it loads (TouchEngine), and `callbacks 0 <dat>` names its callback DAT. `.enginemap` records the I/O wiring. So "this file loads another (untrusted) component" is a static, build-confirmed signal. (`COMP:engine` ×5, `enginemap` ×5; also `engineout` TOP/CHOP/POP/DAT variants.)
...
- **POP-family coverage gap (CLOSED 2026-06-01):** the prior 45-sample corpus had **zero** `POP:` operators, but the shipped-OPSnippets sweep (build 2025.32424; see "Corpus sweep 2026-06-01" above) exercises the full `POP:` family and round-trips bit-exact. Deep decoding of POP-specific body kinds (`.particle`, `.actorType` are candidates) is still open, but the structural gap is closed.
