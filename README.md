...
tox/                     ← text expansion of td-snapshot.tox via expandtoe, for git diffing
```

After editing `src/core.py`, run `./build.sh` to regenerate `td-snapshot.py`. Then export a fresh `td-snapshot.tox` from TouchDesigner (right-click the Container COMP > **Save Component**).

The `tox/` directory is a text expansion of the `.tox` generated by TD's **expandtoe** utility. It has no runtime use — it exists so git can diff the binary `.tox` as readable text.
