...
src/
  ...
  tocdir/                ← Python package: bit-exact parser/emitter for the toeexpand .tox/.toe format (see "toeexpand parser" below)
scripts/
...
`tests/_stress/` is a wide-corpus round-trip validator: it walks a directory of `.tox`/`.toe` files, runs the TD `toeexpand` binary on each, then drives `Project.from_dir().to_dir()` and byte-diffs the result against the original expansion.

```
uv run tests/_stress/run.py                    # default corpus
...
The latest validated run round-trips **1630/1630** files across TD builds `2016.5580` → `2025.31550`. Outputs (`state.db`, `report.md`, `work/`) are gitignored. See [`tests/_stress/README.md`](tests/_stress/README.md) for flags (`--jobs`, `--timeout`, `--toeexpand`/`TOEEXPAND_BIN`).
